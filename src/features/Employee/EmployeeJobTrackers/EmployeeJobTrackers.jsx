...
function EmployeeJobTracker() {
  ...
  // const employeeId = authUser?._id || localStorage.getItem("employeeId") || localStorage.getItem("userId");
  ...
  // Debug: Log the employee ID and jobs data
  useEffect(() => {
    console.log("Employee ID:", employeeId);
    console.log("Jobs data:", job);
  }, [employeeId, job]);

  // Filter jobs for the current employee with fallback options
  const employeeJobs = (job?.jobs || []).filter((j) => {
    // Check multiple possible fields for employee assignment
    const assignedUserId = j.assignedTo?.userId || j.assign;
    return assignedUserId === employeeId;
  });
  ...
  const filteredJobs = employeeJobs.filter((j) => {
    ...
    // If no search terms, just apply the filters
    if (terms.length === 0) {
      ...
      {!loading && !error && employeeJobs.length === 0 && (
        <div className="alert alert-info" role="alert">
          {employeeId
            ...
            : "No employee ID detected. Please make sure you're logged in correctly."}
        </div>
      )}
