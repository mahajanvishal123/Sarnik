...
//     navigate(`/production/OvervieJobsTracker`, { state: { job: jobWithAssignment } });
//   };
//   const handleCheckboxChange = (jobId) => {
//     setSelectedJobs((prev) => ({
//       ...prev,
...
//   // With this:
//   const productionJobs = productionAssignments.flatMap(assignment => {
//     return assignment.jobs?.map(jobItem => {
//       // Add assignment information to each job
//       const jobWithAssignment = { ...jobItem.jobId };
//       jobWithAssignment.assignedTo = {
//         firstName: assignment.employeeId?.firstName || '',
//         lastName: assignment.employeeId?.lastName || '',
//         assign: assignment.employeeId?.assign || ''
//       };
//       return jobWithAssignment;
//     }) || [];
//   });

//   console.log("Production jobs:", productionJobs);

//   // Get designer assignments to filter out in-progress designer jobs
//   const designerAssignments = (assigns?.assignments || []).filter(assignment => {
//     return assignment.employeeId && assignment.employeeId.role === "designer";
//   });
//   console.log("Designer assignments:", designerAssignments);

//   // Extract job IDs from designer assignments
//   const designerJobIds = new Set();
//   designerAssignments.forEach(assignment => {
//     if (assignment.jobs && Array.isArray(assignment.jobs)) {
//       assignment.jobs.forEach(jobItem => {
//         if (jobItem.jobId && jobItem.jobId._id) {
//           designerJobIds.add(jobItem.jobId._id);
//         }
//       });
//     }
//   });
//   console.log("Designer job IDs:", designerJobIds);
...
//     // Check if job is assigned to production
//     const isProductionAssigned = productionJobIds.has(j._id);

//     // Check if job is in progress
//     const isInProgress = (j.Status?.toLowerCase() === "in progress" || j.Status?.toLowerCase() === "in_progress");

//     // Check if job is assigned to designer
//     const isDesignerAssigned = designerJobIds.has(j._id);

//     // Filter logic:
//     // 1. Show if assigned to production (regardless of status)
//     // 2. Show if in progress AND assigned to production
//     // 3. DON'T show if in progress AND assigned to designer
//     const shouldShow = isProductionAssigned &&
//       (!isInProgress || (isInProgress && !isDesignerAssigned));
...
function NewJobsList() {
  ...
  const [selectedStage, setSelectedStage] = useState("All Stages");
  const [showModal, setShowModal] = useState(false);
  const [inProgressJobs, setInProgressJobs] = useState([]); // New state for in-progress jobs
  const [loadingInProgress, setLoadingInProgress] = useState(false); // Loading state for in-progress jobs
  ...
  useEffect(() => {
    dispatch(fetchjobs());
    fetchInProgressJobs(); // Fetch in-progress jobs
  }, [dispatch]);

  // Function to fetch in-progress jobs
  ...
  // Create a Set of in-progress job IDs for easy lookup
  const inProgressJobIds = new Set(inProgressJobs.map(job => job._id));
  ...
  const filteredJobs = productionJobs.filter((j) => {
    ...
    // Filter logic:
    // 1. Show if assigned to production
    // 2. DON'T show if in progress (from the API)
    const shouldShow = isProductionAssigned && !isJobInProgress;
    ...
    dispatch(createAssigns(payload))
      ...
      .then((response) => {
        ...
        if (response.success) {
          ...
          setShowAssignModal(false);
          setSelectedJobs(false);
          // Refresh in-progress jobs after assignment
          fetchInProgressJobs();
        } else {
          setShowAssignModal(false);
          toast.error(response.message || "Assignment failed!");
  ...
  return (
    <div className="container-fluid bg-white p-3 mt-4 rounded shadow-sm">
      ...
      {/* Pagination */}
      {!loading && !loadingInProgress && !error && filteredJobs.length > 0 && (
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div className="text-muted small">
            Showing {(currentPage - 1) * itemsPerPage + 1} to {(currentPage - 1) * itemsPerPage + paginatedProjects.length} of {filteredJobs.length}
          </div>
