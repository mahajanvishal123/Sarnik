...
function CostEstimates({projectNO}) {
  ...
  const handleSavePO = async () => {
    ...
    try {
      ...
      if (createReceivablePurchase.fulfilled.match(result)) {
        ...
        navigate("/production/receivable");
      } else {
        Swal.fire({
          icon: 'error',
  ...
  const renderAddPOModal = () => (
    ...
    const Duplicate = (po) => {
      navigate(`/production/AddCostEstimates`, {
        state: {
          po: {
    ...
    const CostEstimatesDetails = (po) => {
      navigate(`/production/OvervieCostEstimates`, { state: { po } });
    };

    // const Addcostestimate = () => {
    //   navigate(`/production/AddCostEstimates`, { state: {projectNO} });
    // }
    ...
    return (
      <div
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {/* <Link to={"/production/AddCostEstimates"}> */}
            <button id="btn-All" onClick={()=> {navigate(`/production/AddCostEstimates`, { state: {idProject : projectID} })}} className=" btn-dark" style={{ border: "none", borderRadius: "10px" }}>
                ...
                <tr style={{ whiteSpace: "nowrap" }} key={po.poNumber}>
                  ...
                  <td>
                    <span className={`badge ${getStatusClass(po.CostPOStatus)} px-2 py-1`}>
                      {po.CostPOStatus == 'Pending'? 'open':'active'}
                    </span>
                  </td>
...
//   // Function to map CE status based on PO status
//   const mapCEStatus = (poStatus, currentStatus) => {
//     if (!poStatus) return currentStatus;
...
//                 // Get the actual PO status from receivablePurchases if available
//                 const actualPOStatus = po.receivablePurchases?.[0]?.POStatus || po.CostPOStatus || "N/A";
//                 // Map the PO status to display value
//                 const displayPOStatus = mapPOStatus(actualPOStatus);
//                 // Map the CE status based on PO status
//                 const displayCEStatus = mapCEStatus(actualPOStatus, po.Status);
...
//                     </td>
//                     <td>
//                       <span className={`badge ${getStatusClass(displayPOStatus)} px-2 py-1`}>
//                         {displayPOStatus}
//                       </span>
//                     </td>
                 
//                     <td>
//                       <span className={`badge ${getStatusClass(displayCEStatus)} px-2 py-1`}>
//                         {displayCEStatus}
//                       </span>
//                     </td>
//                     <td>
//                       <div className="d-flex gap-2">
//                         <button className="btn btn-sm btn-success"
//                           // Disable button if PO already exists and status is not "Pending"
//                           disabled={
