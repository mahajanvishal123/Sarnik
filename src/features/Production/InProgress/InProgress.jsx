...
// //   // Function to check if a user has the "production" role
// //   const isProductionUser = (assignedTo) => {
// //     if (!assignedTo) return false;

// //     // If assignedTo is an object with role property
// //     if (assignedTo.role) {
// //       return assignedTo.role === "production";
// //     }

// //     // If assignedTo is just an ID, look up in designers array
// //     if (typeof assignedTo.userId === 'string') {
// //       const user = designers.find(designer => designer.id === assignedTo.userId || designer._id === assignedTo.userId);
// //       return user && user.role === "production";
// //     }
...
// //   const filteredProjects = (job?.jobs || []).filter((j) => {
// //     // First check if the assigned user has the "production" role
// //     if (isProductionUser(j.assignedTo)) {
// //       return false;
// //     }
...
// //               <Form.Select onChange={(e) => handleDesignerChange(e.target.value)}>
// //                 <option value="">Select Assign...</option>
// //                 {designers.filter((item) => item.role === "production").map((designer) => (
// //                   <option key={designer.id} value={designer.id}>
// //                     {designer.firstName}{designer.lastName}
// //                   </option>
// //                 ))}
...
// //             </Form.Group>
// //           )}
// //           <button className="btn btn-secondary mt-3" onClick={() => setShowDesignerModal(false)}>
// //             Save
// //           </button>
// //         </Modal.Body>
// //       </Modal>
...
// //             </Dropdown.Item>
// //             {[...new Set(
// //               (job?.jobs || []).map((j) => j.projectId?.[0]?.projectName || "N/A")
...
// //               .reverse()
// //               .filter(
// //                 (job) => job.Status?.toLowerCase().trim() === "in progress"
// //               )
// //               .map((job) => (
// //                 <tr key={job._id}>
// //                   <td>
...
// //                   <td>{job.projectId?.[0]?.projectNo || "N/A"}</td>
// //                   <td>{job.brandName}</td>
// //                   <td>{job.subBrand}</td>
// //                   <td>{job.flavour}</td>
// //                   <td>{job.packType}</td>
...
// //             {(currentPage - 1) * itemsPerPage + paginatedProjects.length} of{" "}
...
// import { useEffect, useState } from "react";
// import { Form, Table, Modal, Dropdown, Button } from "react-bootstrap";
// import { Link, useLocation, useNavigate, useParams } from "react-router-dom";
// import { useDispatch, useSelector } from "react-redux";
// import { filterStatus, Project_job_Id } from "../../../redux/slices/JobsSlice";
...
//   // Open assign modal when clicking employee name
...
//       await dispatch(filterStatus(Status));
...
//       try {
//         const response = await axios.get(`${apiUrl}/user/getAllUsers`);
//         // Fixed: Check if response.data.data exists and has users array
...
//   // Fixed: Ensure we have an array to work with
//   const jobsArray = job?.jobs || job || [];

//   const filteredProjects = jobsArray.filter((j) => {
//     const terms = searchQuery.trim().split(/\s+/).filter(Boolean);
//     if (terms.length === 0) {
//       const matchesProject =
//         selectedProject === "All Projects" ||
...
//               All Projects
//             </Dropdown.Item>
//             {[...new Set(jobsArray.map((j) => j.projectId?.[0]?.projectName || "N/A"))].map(
//               (projectName, index) => (
//                 <Dropdown.Item key={index} onClick={() => setSelectedProject(projectName)}>
//                   {projectName}
//                 </Dropdown.Item>
...
//                         onClick={() => handleDesignerClick(job)}
//                       >
//                         {job.assignedTo ? `${job.assignedTo.firstName} ${job.assignedTo.lastName}` : (job.assign || "Assign")}
//                       </span>
//                     </td>
//                     <td>
//                       {new Date(job.updatedAt).toLocaleTimeString('en-US', {
...
function InProgress() {
  ...
  const [designers, setDesigners] = useState([]);
  const [loadingDesigners, setLoadingDesigners] = useState(false);
  const [productionUsers, setProductionUsers] = useState([]); // New state for production users
  const [loadingProductionUsers, setLoadingProductionUsers] = useState(false); // Loading state for production users
  const { job, loading, error } = useSelector((state) => state.jobs);
  const [Status, setStatus] = useState("In Progress");
  ...
  useEffect(() => {
    const fetchDesigners = async () => {
      ...
      try {
        ...
        // Filter production users
        const productionUsersData = usersData.filter(user =>
          user.role?.toLowerCase() === "production" || user.assign?.toLowerCase() === "production"
        );
        setProductionUsers(productionUsersData);
      } catch (error) {
        console.error("Error fetching designers:", error);
      } finally {
        setLoadingDesigners(false);
  ...
  // Create a Set of production user IDs for easy lookup
  const productionUserIds = new Set(productionUsers.map(user => user._id));

  // Filter out jobs assigned to production users
  const filteredProjects = jobsArray.filter((j) => {
    // Check if job is assigned to a production user
    const isAssignedToProduction = j.assign && productionUserIds.has(j.assign);
    // If assigned to production, exclude it from this list
