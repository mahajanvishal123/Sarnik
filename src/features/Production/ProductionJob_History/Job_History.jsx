...
function Job_History() {
    ...
    ? assigns.assignments
      ...
      .flatMap((assignment) =>
        assignment.jobs.map((jobItem) => ({
          ...
          assignmentId: assignment._id,
          employeeInfo: assignment.employeeId,
          // Add assigned employee information
          assignedEmployee: jobItem.assignedTo || {
  ...
  const filteredJobs = allJobs.filter((j) => {
    ...
    const fields = [
      ...
      (j.packSize || "").toLowerCase(),
      (j.packCode || "").toLowerCase(),
      // Also search by assigned employee name
      (j.assignedEmployee?.firstName || "").toLowerCase(),
      (j.assignedEmployee?.lastName || "").toLowerCase(),
    ];
  ...
  return (
    ...
    >
      ...
      <div className="filters d-flex flex-wrap gap-2 mb-4">
        <Form.Control
          type="search"
          placeholder="Search by Job #, Project Name, Brand, Assignee, etc..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          style={{ width: "250px" }}
        />
        ...
        <Table hover className="align-middle sticky-header">
          <thead className="bg-light">
            <tr>
              ...
              <th>Priority</th>
              <th>Due Date</th>
              <th>Assigned To</th> {/* Changed from "Assign" to "Assigned To" */}
              <th>TimeLogged</th>
              <th>Status</th>
            </tr>
          </thead>
                ...
                <tr key={job._id}>
                  ...
                  <td style={{ whiteSpace: "nowrap" }}>
                    ...
                    {job.assignedEmployee
                      ? `${job.assignedEmployee.firstName || ''} ${job.assignedEmployee.lastName || ''}`.trim()
                      : "Unassigned"}
                  </td>
                  <td>
                    {new Date(job.updatedAt).toLocaleTimeString("en-US", {
