...
//         .then(() => {
//           toast.success("Estimates created successfully!");
//           localStorage.setItem("openFinanceTab", "true");
//           navigate(-1);
//         })
//         .catch(() => toast.error("Failed to create estimates"));
//     } else {
...
//   // Modified function to get all projects or filter based on estimates
...
//     // If no state is passed, return all projects
//     if (!projectNo && !id && !isDuplicate) {
//       console.log("No state passed, returning all projects");
//       return project.data;
//     }
...
//   const reversedProjectList = getProjectList().slice().reverse() || [];
...
function AddCostEstimates() {
  ...
  const { Clients, clientLoading } = useSelector((state) => state.client);
  const { estimates, loading, error } = useSelector(
    (state) => state.costEstimates
  );
  ...
  useEffect(() => {
    console.log("Projects data:", project?.data);
    console.log("Cost estimates data:", estimates.costEstimates);
    console.log("projectNo from location state:", projectNo);
  }, [project, estimates.costEstimates, projectNo]);
  ...
  const getAvailableProjects = () => {
    ...
    // If projectNo is passed, make sure that project is included in the list
    if (projectNo) {
      const requestedProject = project?.data?.find(p => p._id === projectNo);
      if (requestedProject && !reversedProjectList.find(p => p._id === projectNo)) {
        return [requestedProject, ...reversedProjectList];
      }
    }
  ...
  // 🔹 Auto-select project + client if projectNo passed
  useEffect(() => {
    console.log("Auto-select project effect running");
    console.log("projectNo:", projectNo);
    console.log("project data available:", !!project?.data);
    console.log("projectLoading:", projectLoading);
    console.log("clientLoading:", clientLoading);

    if (projectNo && project?.data?.length && !projectLoading && !clientLoading) {
      console.log("Attempting to find project with ID:", projectNo);
      const foundProject = project?.data?.find((p) => p?._id == projectNo);
      ...
      if (foundProject) {
        ...
        const clientObj = Clients?.data?.find((c) => c._id === clientId);
        console.log("Found client:", clientObj);

        setFormData((prev) => {
          console.log("Updating form data with project and client");
          return {
            ...prev,
            projectId: foundProject._id,
            projectName: foundProject.projectName,
  ...
  }, [projectNo, project?.data, Clients?.data, today, projectLoading, clientLoading]);
