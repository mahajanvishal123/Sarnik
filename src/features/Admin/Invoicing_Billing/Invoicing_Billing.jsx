...
// // Currency units and conversion functions
// const currencyUnits = {
...
// // Add this component for the HTML invoice template with exact formatting
// const InvoiceTemplate = ({ invoiceData, companyInfo }) => {
//   // Calculate values
//   const itemsRaw = Array.isArray(invoiceData?.lineItems) ? invoiceData.lineItems : [];
//   const items = itemsRaw.length
//     ? itemsRaw.map((it, i) => ({ ...it, id: i + 1 }))
...
//     try {
//       // Create a container div for our invoice
//       const container = document.createElement('div');
//       container.style.position = 'absolute';
//       container.style.left = '-9999px';
//       container.style.width = '210mm';

//       // Render the InvoiceTemplate component to the container
//       const template = React.createElement(InvoiceTemplate, {
...
//     try {
//       // Try to get the PDF from the API first
//       const response = await axiosInstance.get(`/pdf/invoice?InvoiceBillingId=${row._id}`, { responseType: 'blob' });
...
//         a.remove();
//       } else {
//         // If it's not a PDF, try to parse it as JSON and generate the PDF ourselves
//         const reader = new FileReader();
//         reader.onload = async () => {
//           try {
//             const json = JSON.parse(reader.result);
...
//           } catch {
//             Swal.fire('Error', 'Invalid response received while generating PDF.', 'error');
//           }
//         };
//         reader.readAsText(response.data);
//       }
//     } catch (e) {
//       console.warn('API PDF fallback to client render:', e?.message || e);
//       // If the API fails, generate the PDF ourselves
...
const currencies = [
  { label: "USD - US Dollar", value: "USD", symbol: "$", name: "US Dollar", minor: "Cents", minorFraction: 100 },
  { label: "EUR - Euro", value: "EUR", symbol: "€", name: "Euro", minor: "Cents", minorFraction: 100 },
  { label: "INR - Indian Rupee", value: "INR", symbol: "₹", name: "Indian Rupee", minor: "Paise", minorFraction: 100 },
  ...
  { label: "AED - UAE Dirham", value: "AED", symbol: "د.إ", name: "UAE Dirham", minor: "Fils", minorFraction: 100 },
  { label: "SAR - Saudi Riyal", value: "SAR", symbol: "﷼", name: "Saudi Riyal", minor: "Halalas", minorFraction: 100 },
];

// Create currency names mapping
const currencyNames = currencies.reduce((acc, currency) => {
  acc[currency.value] = {
    major: currency.name,
    minor: currency.minor,
    minorFraction: currency.minorFraction
  };
  return acc;
}, {});
...
const getCurrencyDetails = (currencyValue) => {
  return currencies.find(currency => currency.value === currencyValue) || currencies[0]; // Default to USD
};
...
const InvoiceTemplate = ({ invoiceData, companyInfo, refProp }) => {
  ...
  // Get currency details from invoice data
  ...
  const numberToWords = (value, currencyCode = 'USD') => {
    ...
    const curr = currencyNames[currencyCode] || currencyNames['USD'];
    const dollars = Math.floor(value);
    const cents = Math.round((value - dollars) * curr.minorFraction);
    let words = `${curr.major} ${toWords(dollars)}`;
    if (cents > 0 && curr.minor) {
      words += ` and ${toWords(cents)} ${curr.minor}`;
    }
    return words + ' Only';
  };
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style={{
                  ...
                  padding: '5px',
                  textAlign: 'right'
                }}>{invoiceCurrency} {vatAmt.toFixed(2)}</td>
              </tr>
              <tr>
                <td style={{
                  border: '0.6px solid #000',
                ...
                <td style={{
                  ...
                  padding: '5px',
                  textAlign: 'right'
                }}>{invoiceCurrency} {grandTotal.toFixed(2)}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function Invoicing_Billing() {
  ...
  const handleDownloadPDF = async (row) => {
    ...
    // Set the current invoice data for the template
