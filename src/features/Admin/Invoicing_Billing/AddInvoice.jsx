...
function AddInvoice() {
  ...
  useEffect(() => {
    if (isFromPurchaseOrder && purchaseOrderData) {
      console.log("Purchase order data received:", purchaseOrderData);

      // Get the line items from the purchase order
      const lineItems = purchaseOrderData.lineItems || [];
      console.log("Line items from PO:", lineItems);

      // Set the items state with the line items from the purchase order
      if (lineItems.length > 0) {
        ...
        console.log("Items state set with line items");
      } else {
        setItems([{ description: "", quantity: 0, rate: 0, amount: 0 }]);
        console.log("No line items found, using empty item");
      }
