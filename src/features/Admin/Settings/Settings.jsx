...
//           setLogoPreview(data.companyInfo.logoUrl);
//         }
//         if (data.companyInfo?.stampUrl) {
//           setStampPreview(data.companyInfo.stampUrl);
//         }
...
//       formDataToSend.append('assign', formData.assign);
//       formDataToSend.append('role', userData.role || "Admin");
//       formDataToSend.append('permissions', JSON.stringify(userData.permissions || { dashboardAccess: true, clientManagement: true }));
//       formDataToSend.append('accessLevel', JSON.stringify(userData.accessLevel || { fullAccess: true }));

//       // Add company info as JSON string
//       const companyInfoToSend = {
//         ...formData.companyInfo,
//         // Remove logo and stamp URLs as they'll be handled separately
//         logoUrl: undefined,
//         stampUrl: undefined
//       };
//       formDataToSend.append('companyInfo', JSON.stringify(companyInfoToSend));

//       // Add tax categories as JSON string
//       formDataToSend.append('TaxCategories', JSON.stringify(formData.TaxCategories));
...
//       }
//       if (companyStamp) {
//         formDataToSend.append('companyStamp', companyStamp);
//       }

//       const response = await fetch(`${apiUrl}/user/685e6f6364a81f874cd99761`, {
//         method: "PATCH",
//         body: formDataToSend
//         // Don't set Content-Type header when using FormData, the browser will set it automatically with the correct boundary
//       });
...
//       // Update form data with new URLs if they were returned
//       if (updatedData.companyInfo) {
//         setFormData(prev => ({
//           ...prev,
//           companyInfo: {
//             ...prev.companyInfo,
//             logoUrl: updatedData.companyInfo.logoUrl || prev.companyInfo.logoUrl,
//             stampUrl: updatedData.companyInfo.stampUrl || prev.companyInfo.stampUrl
//           }
//         }));

//         // Update previews
//         if (updatedData.companyInfo.logoUrl) {
//           setLogoPreview(updatedData.companyInfo.logoUrl);
//         }
//         if (updatedData.companyInfo.stampUrl) {
//           setStampPreview(updatedData.companyInfo.stampUrl);
//         }
//       }
...
const SettingsPage = () => {
  ...
  // Function to resize an image to specific dimensions (stretch to fill)
  ...
  const handleLogoChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      // Resize logo to exact invoice dimensions (522px width, 65px height)
      resizeImage(file, 522, 65, (resizedBlob) => {
        setCompanyLogo(resizedBlob);
        setLogoPreview(URL.createObjectURL(resizedBlob));
      });
    }
  };
  ...
  const handleStampChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      // Resize stamp to exact invoice dimensions (169px width, 113px height)
      resizeImage(file, 169, 113, (resizedBlob) => {
        setCompanyStamp(resizedBlob);
        setStampPreview(URL.createObjectURL(resizedBlob));
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add files if they exist
      if (companyLogo) {
        formDataToSend.append('companyLogo', companyLogo, 'logo.png');
      }
      if (companyStamp) {
        formDataToSend.append('companyStamp', companyStamp, 'stamp.png');
      }
      const response = await fetch(`${apiUrl}/user/685e6f6364a81f874cd99761`, {
        method: "PATCH",
        body: formDataToSend
                      ...
                      <Row className="mb-4">
                        <Col md={12}>
                          <Form.Group className="mb-3">
                            ...
                            <Form.Text className="text-muted d-block mb-2">
                              Required size: 522px × 65px (image will be stretched to fit)
                            </Form.Text>
                            {/* {logoPreview && (
                              <div className="mb-2 border p-2" style={{ backgroundColor: '#D21015' }}>
                                <img
                                  src={logoPreview}
                                  alt="Company Logo"
                                  style={{ width: "522px", height: "65px", display: 'block' }}
                                />
                              </div>
                            )} */}
                            {logoPreview && (
                              <div
                                className="mb-2 border p-2 d-flex justify-content-center align-items-center"
                                style={{ width: "522px", height: "65px", backgroundColor: "#f8f9fa" }}
                              >
                                <img
                                  src={logoPreview}
                                  alt="Company Logo"
                        ...
                        <Col md={12}>
                          <Form.Group className="mb-3">
                            <Form.Label>Company Stamp</Form.Label>
                            <Form.Text className="text-muted d-block mb-2">
                              Required size: 169px × 113px (image will be stretched to fit)
                            </Form.Text>
                            {stampPreview && (
                              <div
                                className="mb-2 border p-2 d-flex justify-content-center align-items-center"
                                style={{ width: "169px", height: "113px", backgroundColor: "#f8f9fa" }}
                              >
                                <img
                                  src={stampPreview}
                                  alt="Company Stamp"
